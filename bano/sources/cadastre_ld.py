...
def download(departement):
    ...
    resp = requests.get(f'https://cadastre.data.gouv.fr/data/etalab-cadastre/latest/geojson/departements/{departement}/cadastre-{departement}-lieux_dits.json.gz', headers=headers)
...
def import_to_pg(departement, **kwargs):
    ...
    with gzip.open(fichier_source, mode='rt') as f:
        ...
        with  db.bano_cache.cursor() as cur_insert:
            try:
                cur_insert.execute(f"DELETE FROM lieux_dits WHERE insee_com LIKE '{departement+'%'}';COMMIT;")
                a_values = []
