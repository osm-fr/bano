...
for i, d in enumerate(a_0_depts):
    ...
    request = urllib2.Request(
        "http://www.cadastre.gouv.fr/scpc/listerCommune.do?CSRF_TOKEN={:s}&codeDepartement={:s}&libelle=&keepVolatileSession=&offset=5000".format(token, d))
    response = opener.open(request)
    rep = response.read()
    fw = open(fname, 'wb')
    fw.write(rep)
    ...
    for line in fr:
        ...
        if len(lsplit) > 1:
            ...
            commune_cp = lsplit[0].split('td class="nom"')[1].split('feuille')[
                0].split('strong')[1]
            nom_commune = commune_cp[1:-11]
            cp = commune_cp[-9:-4]
            f_output.write('{:s},{:s},{:s},{:s},{:s},{:s}\n'.format(
                a_depts[i], d, nom_commune, cp, code_commune, format))
        f_output.flush()
    fr.close()
    os.remove(fname)
f_output.close()
