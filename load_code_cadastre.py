...
for i,d in enumerate(a_0_depts):
	...
	time.sleep(1)
	opener.open(request)
	request = urllib2.Request("http://www.cadastre.gouv.fr/scpc/listerCommune.do?CSRF_TOKEN={:s}&codeDepartement={:s}&libelle=&keepVolatileSession=&offset=5000".format(token,d))
	response = opener.open(request)
	rep = response.read()
	fw = open(fname,'wb')
	fw.write(rep)
