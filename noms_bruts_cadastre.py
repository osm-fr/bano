...
#    code_insee = get_code_insee_from_cadastre(code_commune)
    ...
    for r in osm.relations.itervalues():
        ...
        a_values_brut.append("('{:s}','{:s}','{:s}')".format(code_insee,r.tags['name'].encode('utf8').replace("'","''"),fantoir))

    sload = "DELETE FROM cadastre_noms_bruts WHERE insee_com = '{:s}';INSERT INTO cadastre_noms_bruts (insee_com,voie_cadastre,fantoir) VALUES".format(code_insee)
